/**
@file simulation_controller.tpp
@brief Implementation of SimulationController.
*/
#pragma once
...
void SimulationController<ArbiterT>::setup_mesh(int width, int height) {
    interconnect_ = std::make_unique<MeshInterconnect<ArbiterT>>(width, height);
}

template <typename ArbiterT>
void SimulationController<ArbiterT>::build() {
    if (!interconnect_) {
...
template <typename ArbiterT>
void SimulationController<ArbiterT>::create_nodes() {
    if (!interconnect_) return;
    nodes_.clear();
    int num_nodes = interconnect_->get_total_nodes();
    for (int i = 0; i < num_nodes; ++i) {
        nodes_.push_back(std::make_unique<NetworkNode<ArbiterT>>(i, *interconnect_, trace_));
    }
}
...
//=============================================================================
template <typename ArbiterT>
void SimulationController<ArbiterT>::generate_uniform_trace(int total_packets, double injection_rate, int max_ticks) {
    if (!interconnect_) {
...
template <typename ArbiterT>
void SimulationController<ArbiterT>::generate_hotspot_trace(int total_packets, int hotspot_node,
...
template <typename ArbiterT>
void SimulationController<ArbiterT>::generate_synthetic_trace(const std::string& pattern) {
    if (!interconnect_) {
        throw std::runtime_error("Topology not set up.");
    }
    int num_nodes = interconnect_->get_total_nodes();
...
template <typename ArbiterT>
void SimulationController<ArbiterT>::load_trace(const std::string& filename) {
    trace_.load_from_json(filename);
    create_nodes();
}

template <typename ArbiterT>
void SimulationController<ArbiterT>::save_trace(const std::string& filename) const {
...
//=============================================================================
template <typename ArbiterT>
void SimulationController<ArbiterT>::run(int max_ticks) {
    if (!is_built_) {
        build();
    }
...
template <typename ArbiterT>
void SimulationController<ArbiterT>::step() {
    if (!is_built_) {
        build();
    }
    for (auto& node : nodes_) {
...
//=============================================================================
template <typename ArbiterT>
void SimulationController<ArbiterT>::dump_results() const {
...
template <typename ArbiterT>
void SimulationController<ArbiterT>::save_results(const std::string& filename) const {
    json results;
    // Общая статистика

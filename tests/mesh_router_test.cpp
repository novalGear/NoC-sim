...
@date 18/03/2026
@updated 10/05/2026 - Adapted for templated MeshRouter<ArbiterT> and fixed type casts
*/
...
protected:
    void SetUp() override {
        ...
        // Явно указываем тип арбитра RRArbiter
        center_router = std::make_unique<MeshRouter<RRArbiter>>(4, 1, 1, 3, 3);
        corner_router = std::make_unique<MeshRouter<RRArbiter>>(0, 0, 0, 3, 3);
        edge_router   = std::make_unique<MeshRouter<RRArbiter>>(1, 1, 0, 3, 3);

        // Создаем роутеры для линейных сеток
        horiz_router = std::make_unique<MeshRouter<RRArbiter>>(2, 2, 0, 5, 1);
        vert_router  = std::make_unique<MeshRouter<RRArbiter>>(2, 0, 2, 1, 5);
    ...
    void register_all_ports(MeshRouter<RRArbiter>* router) {
    ...
    void register_local_ports(MeshRouter<RRArbiter>* router) {
        auto in  = std::make_unique<Port>();
        auto out = std::make_unique<Port>();
    ...
    std::unique_ptr<MeshRouter<RRArbiter>> center_router;
    std::unique_ptr<MeshRouter<RRArbiter>> corner_router;
    std::unique_ptr<MeshRouter<RRArbiter>> edge_router;
    std::unique_ptr<MeshRouter<RRArbiter>> horiz_router;
    std::unique_ptr<MeshRouter<RRArbiter>> vert_router;
...
TEST_F(MeshRouterTest, InjectPacketToLocalPort) {
    auto router = std::make_unique<MeshRouter<RRArbiter>>(4, 1, 1, 3, 3);
...
TEST_F(MeshRouterTest, PacketMovesFromLocalInToEastOut) {
    auto router = std::make_unique<MeshRouter<RRArbiter>>(0, 0, 0, 3, 3);
    register_local_ports(router.get());
